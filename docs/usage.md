...
`family-plan` 只生成治理 workflow 和 coverage discipline。默认 governed repo set 是 OPL、MAS、MAG、RCA、OMA、BookForge、App 七个核心 repo；`opl-doc`、`opl-aion-shell` 等 support repo 只作为 explicit extension 出现在 `support_repo_policy`，不是默认 Foundry Agent truth set。
