...
15. `family-plan` 的 11 仓/22 个概念主参考只是内置 workflow baseline，不是 live family inventory。每轮必须按 workspace 中实际存在的 OPL-owned repo、repo-local owner 与用户范围扩展或收缩；已退役/不存在仓库不形成缺失项，上游 fork 主体不得混入治理写集。
...
25. 多仓治理必须在修改前 fresh 检查 branch/head、dirty、worktree、ahead/behind、remote 与并发 owner write set；每个变更仓独立验证、提交、push 并回读远端 ref。currentness、owner、fork 或验证 gate 失败时必须保留 blocker、owner、合法入口和停止条件；docs、focused tests、queue clean、commit/push 或 tracking-ref clean 不得越级声明 runtime/domain/release/owner-acceptance/production ready。
