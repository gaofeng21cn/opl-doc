...
- `scripts/opl_doc_doctor.py family-plan`：OPL series 治理工作流生成，内置 `one-person-lab`、`one-person-lab-app`、`opl-native-workbench`、`opl-flow`、`opl-doc`、`med-autoscience`、`med-autogrant`、`redcube-ai`、`opl-meta-agent`、`opl-bookforge`、`mas-scholar-skills` 这 11 仓 baseline，并通过当前 workspace/repo-local owner 盘点与 `--repo ID=PATH` 扩展到实际存在的 OPL-owned 产品或分发仓；baseline 不是 family truth owner set。
