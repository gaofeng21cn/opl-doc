...
- `scripts/opl_doc_doctor.py family-plan`：OPL series 治理工作流生成，内置 `one-person-lab`、`one-person-lab-app`、`opl-native-workbench`、`opl-flow`、`opl-doc`、`med-autoscience`、`med-autogrant`、`redcube-ai`、`opl-meta-agent`、`opl-bookforge`、`mas-scholar-skills` 这 11 仓 baseline。机器输出通过 `scope_policy` 明确 repo map / 概念主参考只是 workflow baseline 或显式候选输入；实际运行必须从当前 workspace、repo-local guidance/docs/contracts、owner 标识与用户范围 fresh 发现并冻结 actual governed scope，已退役/不存在 repo 不形成 backlog，upstream fork 主体只读排除。
- `family-plan` 机器 workflow 现在要求修改前 fresh 检查 branch/head、dirty、worktree、ahead/behind、remote 与并发 owner write set，先生成 `governance_worklist` / authority-aware matrix 并优先选择跨仓高价值且写集安全的治理包；每个变更仓独立验证、提交、push 和远端 ref 回读，阻断项保留 blocker、owner、合法入口与停止条件。docs、focused tests、queue clean 或 push 成功仍不能单独推出 runtime/domain/release/owner-acceptance/production ready。
...
- `scripts/opl_doc_doctor.py family-plan`：现在输出由 `contracts/support-repo-policy.json` 同源派生的 `support_repo_policy`，把 `opl-aion-shell` 标为 upstream shell explicit extension；内置 11 仓/22 个概念主参考是 workflow baseline，实际 governed scope 必须按本轮存在的 owner repo 扩展或收缩，upstream shell repos 不进入默认 Foundry Agent truth set。
...
- `family-plan` 对内置 11 仓生成 22 个概念主参考；实际运行只使用目标仓真实声明的 ideal-state/Active Truth owner，并把当前存在的额外 OPL-owned repo 纳入 coverage。不存在或已退役的仓库不要求重建文档，也不形成 backlog。
