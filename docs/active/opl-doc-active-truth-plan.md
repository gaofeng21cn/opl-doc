...
| Active gap no-gap thinning | done | `skills/opl-doc/SKILL.md`; `docs/invariants.md`; `docs/status.md` | Gap docs are now explicitly treated as active trackers, not completion ledgers. Closed functional/structural gaps must be removed or rewritten from active plans; when no current gap remains, the active plan stays as a thin current-state / no-gap / next-audit baton with history/provenance pointers. |
...
| --- | --- | --- | --- | --- | --- |
| none_selected | OPL Doc governance has one active truth owner, no active compatibility resurrection, repo-temp verification hygiene, support repo extension guard, governance worklist/batch gate, and active gap no-gap thinning rules. | Current functional/structural governance baseline is landed; the landed areas are summarized in `Current Completion Progress`. | No active functional/structural gap is selected. Reopen only from fresh source/tests/CLI/docs truth, not from completed rows. | `skills/opl-doc/SKILL.md`; `docs/invariants.md`; `docs/status.md`; this active plan | `bash scripts/verify.sh`; `git diff --check`; targeted stale scan. |
