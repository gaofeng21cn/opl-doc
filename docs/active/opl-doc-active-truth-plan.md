...
| Support repo policy contract | done | `contracts/support-repo-policy.json`; `scripts/opl_doc_doctor_parts/constants.py`; `scripts/opl_doc_doctor_parts/family_plan.py`; `tests/test_opl_doc_doctor.py` | Support repos are machine-declared as extension-only, excluded from the default governed repo set and Foundry Agent truth set, and guarded against no-resurrection drift. The hyphen path is canonical; `contracts/support_repo_policy.json` is a forbidden legacy ref with no alias file. |
| Support profile no-resurrection guard | done | `scripts/opl_doc_doctor.py support-profile-check . --format json`; `scripts/verify.sh support-profile:strict`; `scripts/opl_doc_doctor_parts/constants.py#build_support_profile_guard`; `scripts/opl_doc_doctor_parts/family_plan.py#build_support_profile_guard_audit`; `scripts/opl_doc_doctor_parts/rendering.py`; `tests/test_opl_doc_doctor.py` | `family-plan` emits both `support_profile_guard` and `support_profile_guard_audit`, and `support-profile-check` now exposes the same audit as a direct strict readback with non-zero failure semantics. The audit reads the materialized native profile, support policy contract, default governed repo set and filesystem legacy-ref absence, and reports 7 fail-closed checks. It proves only profile/workflow no-resurrection guard state, not repo truth, Foundry Agent truth, owner receipt, quality verdict, production readiness, or global goal completion. |
...
| Governance worklist and batch gate | done | `skills/opl-doc/SKILL.md`; `docs/invariants.md`; `templates/goal-opl-family-doc-lifecycle.md`; this document | OPL Doc now requires broad governance to build `governance_worklist` / authority-aware matrix before mutation, apply a minimum effective batch gate, route pure structure items to `refactor_patrol`, preserve selected/skipped/no-safe statuses, and keep upstream fork bodies / support repo extensions out of cleanup pressure. |
...
| --- | --- | --- | --- | --- | --- |
| support-repo-profile-contract | Support repo participates in OPL native profile drift checks without becoming a Foundry Agent truth owner. | `contracts/opl-native-profile.json` is committed for this support repo and declares `opl-doc` as a `codex_plugin` profile with extension-only authority; `contracts/support-repo-policy.json` materializes the extension-only support repo policy and no-resurrection guard used by `family-plan`; `family-plan` emits `support_profile_guard` and `support_profile_guard_audit`; `support-profile-check` exposes the same audit as a strict CLI/readback path; `native-check --format json` now materializes `support_repo_policy`, `support_profile_guard` and `support_profile_guard_audit` alongside profile drift state. The audit verifies the generated policy matches the materialized contract, the native profile declares extension-only support role, default governed repo ids/names exclude support repos, the old `contracts/support_repo_policy.json` path is absent, and false-ready flags fail closed. | Keep `native-check . --format json` in default verification so profile drift and support/no-resurrection readback fail closed together; keep `family-plan` deriving support policy and support profile guard from `build_support_repo_policy()` / `build_support_profile_guard()` and computing `support_profile_guard_audit` from materialized sources; keep `support-profile-check` / `verify.sh support-profile:strict` as the direct no-resurrection executable guard; do not treat either contract, guard or audit as repo truth, Foundry Agent truth, owner receipt, quality verdict or production readiness; do not add legacy underscore contract aliases. | `contracts/opl-native-profile.json`; `contracts/support-repo-policy.json`; `scripts/opl_doc_doctor.py`; `scripts/opl_doc_doctor_parts/constants.py`; `scripts/opl_doc_doctor_parts/family_plan.py`; `scripts/opl_doc_doctor_parts/plugin_sync.py`; `scripts/verify.sh`; `docs/status.md` | `python3 scripts/opl_doc_doctor.py support-profile-check . --format json`; `python3 scripts/opl_doc_doctor.py family-plan --format json`; `python3 scripts/opl_doc_doctor.py family-plan --format markdown`; `python3 scripts/opl_doc_doctor.py native-check . --format json`; `bash scripts/verify.sh support-profile:strict`; `bash scripts/verify.sh`. |
...
| governance-worklist-batch-gate | Multi-repo OPL Doc governance should select work from an authority-aware candidate pool instead of mutating isolated micro-slices. | `skills/opl-doc/SKILL.md`, `docs/invariants.md`, and `templates/goal-opl-family-doc-lifecycle.md` now require a pre-mutation `governance_worklist` / matrix, route/status fields, minimum effective batch gate, dirty write-set ownership, no-safe batch output, and upstream fork exclusion. | Keep the skill, invariants, and goal template synchronized when automation governance rules change; do not add doctor code unless machine-readable `family-plan` output itself needs matrix fields. | `skills/opl-doc/SKILL.md`; `docs/invariants.md`; `templates/goal-opl-family-doc-lifecycle.md`; this active plan | `bash scripts/verify.sh`; `git diff --check`; targeted stale scan for worklist/batch/fork terms. |
