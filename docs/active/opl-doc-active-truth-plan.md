...
- Current support-repo baseline: canonical `opl-doc` skill surface, split doctor implementation, local installer, family governance workflow, native profile sync, support repo policy / no-resurrection guard, repo-temp verification hygiene, governance worklist / batch gate, and active gap no-gap thinning are the maintained structural surface.
