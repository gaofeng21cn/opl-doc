...
`family-plan` 内置 OPL、App、Native Workbench、OPL Flow、OPL Doc、MAS、MAG、RCA、OMA、BookForge 和 MAS Scholar Skills 这 11 仓 workflow baseline，并允许通过 `--repo` 纳入当前实际存在的 OPL-owned 产品或分发仓。该 baseline 只输出治理 workflow、coverage discipline 和 `/goal` objective，不是 live family inventory；已退役或不存在的仓库不构成缺失项。`opl-aion-shell` 等 upstream shell repo 通过 `support_repo_policy` 保持 explicit extension/read-only fork boundary。该输出不持有 repo truth、runtime truth、domain truth、artifact authority、quality verdict、owner receipt、production readiness 或 Foundry Agent truth set。
