...
7. Support extension 层
   - `family-plan` 内置 11 个 OPL series repo 的 workflow baseline；实际治理范围从当前 workspace 与 repo-local owner fresh 发现，并通过 `--repo` 纳入实际存在的额外 OPL-owned 产品或分发仓。`opl-aion-shell` 等 upstream shell repo 只在用户显式要求或当前任务触及 shell carrier / support docs 时作为 read-only extension 纳入；它们不是默认 Foundry Agent truth set。
   - `contracts/support-repo-policy.json` 物化这条 support-extension policy；`family-plan` 从同一 source 派生 `support_repo_policy`。只有 OPL Doc 自仓的 native profile 把该合同列为本仓插件管理面；目标仓 profile 只声明目标仓内实际由 `native-sync` 管理的 `contracts/opl-native-profile.json`，不伪造跨仓 owner/path。该合同是 no-resurrection / no-second-truth guard，不是目标 repo truth 或 Foundry Agent truth owner 集合。
