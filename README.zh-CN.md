...
- **Repo-native 代理上下文**：目标 repo 可以自带 `.opl-doc-governance/`，让后续代理不用长 prompt 也能发现本地治理规则。
...
要让目标仓库对后续代理自描述，可以初始化 repo-native anchor：

```bash
python3 scripts/opl_doc_doctor.py init-repo /path/to/repo --format json
```

这会写入 `.opl-doc-governance/config.json`、`.opl-doc-governance/agent-entry.md` 和 `.opl-doc-governance/README.md`。写入前可用 `--dry-run` 检查。

## 它如何工作

- 如果目标 repo 带 `.opl-doc-governance/agent-entry.md`，代理先读这个本地入口。
...
- `.opl-doc-governance/`：本仓自带的 repo-native 治理锚点。
...
- 本仓保留 OPL-native taxonomy，不把项目迁移到 OpenArc、OpenSpec、Spec Kit 或 Agent OS 的固定布局；`.opl-doc-governance/` 只是本地 agent-readable anchor。
